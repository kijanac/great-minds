...
>
  {#if web}
    <Globe size={9} class="mr-1.5 shrink-0 text-gold-muted" />
  {:else}
    <Search size={9} class="mr-1.5 shrink-0 opacity-60" />
  {/if}
  <span class="min-w-0 truncate"
    >{source.label}{#if documentTitle}<span class="opacity-60">
        · in {documentTitle}</span
      >{/if}</span
  >
</Badge>
