    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <div className="max-w-[740px] mx-auto px-4 md:px-10 pt-8 pb-20">
          ...
          {sourceActions?.notice && (
            <p className="mb-3 px-3 font-mono text-[length:var(--text-chrome)] tracking-[0.04em] text-gold-muted">
              {sourceActions.notice}
            </p>
          )}
...
function SourceActionButton({
  ...
  const pending = actions.busyPath === item.file_path;
  const title = item.title ?? item.file_path;
  ...
  async function handleConfirm() {
    try {
      await confirmedAction.onConfirm(item.file_path);
      setOpen(false);
    } catch {
