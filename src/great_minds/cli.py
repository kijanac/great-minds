...
from great_minds.core.telemetry import emit_wide_event, init_wide_event, setup_logging, wide_event
...
app = typer.Typer(name="great-minds", help="LLM-powered research knowledge base", no_args_is_help=True)
console = Console()
    ...
    try:
        async with _cli_sm.get()() as session:
            ...
            ctx = await pipeline.build_context(
                vault_id=vault_id, storage=make_storage(vault), session=session, client=client,
            )
...
async def compile(  # noqa: A001
    ...
    table = Table(title="Compile Complete", title_style="bold green", border_style="dim blue")
    table.add_column("Metric", style="cyan", no_wrap=True)
    table.add_column("Value", style="white")
    for metric, value in [
        ("Raw chunks indexed", e.get("raw_chunks_indexed", 0)),
        ("Docs extracted", f"{e.get('docs_extracted', 0)} ([dim]{e.get('docs_failed', 0)} failed[/dim])"),
        ("Topics validated", e.get("validated_topics", 0)),
        ("Articles rendered", f"{e.get('render_topics_rendered', 0)} ([dim]{e.get('render_topics_failed', 0)} failed[/dim])"),
...
@app.command()
async def query(
    question: list[str] | None = typer.Argument(None, help="Question text (omit for interactive mode)"),
    model: str | None = typer.Option(None, "--model", help="Override the default model"),
    json_logs: bool = typer.Option(False, "--json-logs", help="Structured JSON logs to stderr"),
) -> None:
    """Query the knowledge base (interactive if no question given)."""
    setup_logging(service="great-minds", json_output=json_logs)
    source = querier.QuerySource(storage=_make_storage(), label="local", vault_id=uuid.UUID(int=0))
    async with _cli_session():
        async with _cli_sm.get()() as session:
            ...
            console.print(Panel.fit(
                ...
                title="[bold]great-minds[/bold]", border_style="blue"))
            while True:
                try:
...
async def ingest(
    ...
    origin: str | None = typer.Option(None, "--origin", help="Publication or organization name"),
    url: str | None = typer.Option(None, "--url", help="Source URL"),
    outlet: str | None = typer.Option(None, "--outlet", help="News outlet (for news type)"),
) -> None:
    ...
    kwargs = {k: v for k, v in (("author", author), ("date", date), ("origin", origin), ("url", url), ("outlet", outlet)) if v}
    ...
    if path.is_file():
        ...
        console.print(f"Found [bold]{total}[/bold] .md files in [cyan]{path}[/cyan] ([dim]{len(existing)} already ingested[/dim])")
        if not total:
        ...
        with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), BarColumn(),
                      TaskProgressColumn(), TextColumn("•"), TimeRemainingColumn(), console=console) as progress:
            task = progress.add_task(f"Ingesting [cyan]{content_type}[/cyan] files", total=total)
            for filepath in source_files:
                ...
                if file_dest in existing:
                    ...
                    continue
                await write_document(storage, config, filepath.read_text("utf-8"), content_type, dest=file_dest, **kwargs)
                ingested += 1
                progress.advance(task)
...
async def reset(
    ...
    if get_settings().storage_backend != "local":
        err_console.print("[bold red]Error:[/bold red] reset is local-backend only; R2 vault reset not yet implemented")
        raise typer.Exit(code=1)
    ...
    if not yes:
        ...
        console.print(Panel.fit(
            ...
            title="[bold yellow]⚠ Reset Vault[/bold yellow]", border_style="red"))
    ...
    async with _cli_session():
        ...
        async with sm() as session:
            for table in ("search_index", "tasks", "source_proposals", "topics", "idea_embeddings", "documents"):
                result = await session.execute(text(f"DELETE FROM {table} WHERE vault_id = :bid"), {"bid": vault_id})
                console.print(f"  [dim]deleted {result.rowcount} rows from[/dim] {table}")
...
async def delete_vault(
    ...
    if not yes:
        ...
        console.print(Panel.fit(
            ...
            title="[bold red]⚠ Delete Vault[/bold red]", border_style="red"))
    ...
    async with _cli_session():
        sm = _cli_sm.get()
        async with sm() as session:
            vault_service = VaultService(VaultRepository(session), UserRepository(session), settings)
            deleted = await vault_service.delete_vault(uuid.UUID(vault_id))
            if deleted is None:
                console.print(f"[yellow]Warning:[/yellow] no vault row found with id={vault_id}")
            else:
                console.print(f"  [dim]vault row deleted and storage cleared for[/dim] {vault_id}")
    if sidecar.exists():
...
def serve(
    ...
    if reload:
        uvicorn.run("great_minds.app.api.server:create_app", factory=True,
                    host=host, port=port, reload=True, reload_dirs=["src"])
    else:
