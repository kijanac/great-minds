"""Brain resolution FastAPI dependencies."""
...
async def resolve_brain(
    ...
    instance = brain_service.get_instance(brain_row)
    ...
    for other_row, _role in user_brains:
        if other_row.id != brain_row.id:
            all_brains.append(brain_service.get_instance(other_row))

    return ResolvedBrain(
        instance=instance,
        row=brain_row,
        manager=brain_service.get_task_manager(brain_row),
