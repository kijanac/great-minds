"""Crawl Sam Marcy's works from marxists.org to local markdown files.

Covers articles, books, and pamphlets from the ETOL (Encyclopedia of
Trotskyism On-Line) writers archive.

Output:
    corpus/marcy/...  — markdown files mirroring the site's URL structure

Usage:
    uv run python scrapers/crawl_marcy.py
"""
...
BASE_URL = "https://www.marxists.org/history/etol/writers/marcy/index.htm"
CORPUS_DIR = Path("corpus/marcy")

ALLOWED_PATTERNS = [
    "*marxists.org/history/etol/writers/marcy/*.htm*",
]
...
def url_to_filepath(url: str) -> Path:
    ...
    path = path.removeprefix("/history/etol/writers/marcy/")
...
async def crawl() -> None:
    """BFS-crawl Marcy's archive and save each page as markdown."""
