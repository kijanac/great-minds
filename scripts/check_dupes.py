...
async def main():
    async with session_maker() as s:
        ...
        if dupes:
            ...
            for row in dupes:
                print(f"  brain={row.brain_id}  path={row.path}  chunk={row.chunk_index}  count={row.n}")
        else:
            print("No duplicate (brain_id, path, chunk_index) combinations found.")

        # Overall stats
        total = await s.execute(text("SELECT count(*) FROM search_index"))
        distinct = await s.execute(text("SELECT count(DISTINCT (brain_id, path, chunk_index)) FROM search_index"))
        print(f"\nTotal rows: {total.scalar()}")
        print(f"Distinct (brain, path, chunk): {distinct.scalar()}")

        # Check how many brains have entries
        brains = await s.execute(text("SELECT brain_id, count(*) FROM search_index GROUP BY brain_id"))
        for row in brains:
