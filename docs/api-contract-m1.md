...
12. Empty `204` responses: Python emits `content-type: application/json` with no body; TS emits no content type. Immaterial (no body to interpret) — the parity harness ignores content type on explicitly-marked no-body mutation entries only.
