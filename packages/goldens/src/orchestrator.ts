...
const cassettePath = join(packageRoot, "cassettes", "python-compile.json");
const goldenPath = join(packageRoot, "goldens", "python-compile.json");
...
const request = async (baseUrl: string, path: string, token: string | undefined, init?: RequestInit) => {
...
const waitForHealth = async (child: Child, url: string) => {
  ...
  while (Date.now() < deadline) {
    if (child.process.exitCode !== null) throw new Error(`backend exited before health check (${child.process.exitCode})`);
    try {
...
const seed = async (dataDir: string, env: NodeJS.ProcessEnv) => {
  ...
  const values = corpus.map((doc) => `(${sqlString(doc.id)},${sqlString(fixtureIds.vault)},${sqlString(doc.path)},${sqlString(fileContentHash(doc.content))},${sqlString(bodyContentHash(doc.content.split("---\n").slice(2).join("---\n")))},'document','golden fixture','{}'::text[],'{}'::jsonb)`).join(",");
  const sql = `
...
const token = async (baseUrl: string) => {
  const body = await request(baseUrl, "/v1/auth/verify-code", undefined, { method: "POST", body: JSON.stringify({ email: "goldens@example.com", code: "000000" }) }) as Row;
  return String(body.access_token);
};

const normalizedSse = async (baseUrl: string, bearer: string, runId: string) => {
  const response = await fetch(`${baseUrl}/v1/vaults/${fixtureIds.vault}/jobs/${runId}/stream`, { headers: { authorization: `Bearer ${bearer}` } });
  ...
  const phasesSeen = [...new Set(messages.map((frame) => String(frame.data?.phase ?? "")).filter((phase) => phase.length > 0))];
  const stepsByPhase = Object.fromEntries(phasesSeen.map((phase) => {
    const steps = new Map<string, string>();
    for (const frame of messages.filter((candidate) => candidate.data?.phase === phase)) {
      for (const step of (frame.data?.steps ?? []) as Row[]) steps.set(String(step.key), String(step.label));
    }
  ...
  return {
    ...
    phasesSeen,
    stepsByPhase,
    terminal: terminal === undefined ? null : {
...
const compile = async (baseUrl: string, bearer: string, runId: string, env: NodeJS.ProcessEnv) => {
  await output("psql", [databaseUrl, "-v", "ON_ERROR_STOP=1", "-c", `insert into pipeline_runs(id,vault_id,trigger,status,current_phase,phase_status) values (${sqlString(runId)},${sqlString(fixtureIds.vault)},'manual','pending','','') on conflict(id) do nothing`], { cwd: repoRoot, env });
  const sse = normalizedSse(baseUrl, bearer, runId);
  await sleep(100);
  await request(baseUrl, `/v1/vaults/${fixtureIds.vault}/compile`, bearer, { method: "POST", body: JSON.stringify({ job_id: runId }) });
  const seen: Json[] = [];
  let previous = "";
  const deadline = Date.now() + 45 * 60_000;
  while (Date.now() < deadline) {
    const row = await output("psql", [databaseUrl, "-At", "-v", "ON_ERROR_STOP=1", "-c", `select jsonb_build_object('status',status,'current_phase',current_phase,'phase_status',phase_status,'progress_steps',progress_steps,'error',error) from pipeline_runs where id=${sqlString(runId)}`], { cwd: repoRoot, env });
    const job = JSON.parse(row.trim()) as Row;
    const progress = JSON.stringify({ phase: job.current_phase, phase_status: job.phase_status, steps: job.progress_steps });
    if (progress !== previous) {
      seen.push(JSON.parse(progress) as Json);
      previous = progress;
    }
    if (["completed", "failed", "cancelled"].includes(String(job.status))) {
      if (job.status !== "completed") throw new Error(`compile ${runId} ended ${String(job.status)}: ${String(job.error)}`);
...
const pinArchiveSuccessor = async (dataDir: string, env: NodeJS.ProcessEnv) => {
  const successor = (await output("psql", [databaseUrl, "-At", "-v", "ON_ERROR_STOP=1", "-c", `select topic_id from topics where vault_id=${sqlString(fixtureIds.vault)} and article_status!='archived' order by slug limit 1`], { cwd: repoRoot, env })).trim();
  ...
  await output("psql", [databaseUrl, "-v", "ON_ERROR_STOP=1", "-c", `update topics set superseded_by=${sqlString(successor)} where topic_id=${sqlString(fixtureIds.archiveTopic)}; update wiki_articles set file_hash=${sqlString(fileContentHash(updated))} where topic_id=${sqlString(fixtureIds.archiveTopic)}`], { cwd: repoRoot, env });
};

const queryJson = async (env: NodeJS.ProcessEnv, query: string): Promise<Json> => {
  const text = await output("psql", [databaseUrl, "-At", "-v", "ON_ERROR_STOP=1", "-c", `select coalesce(jsonb_agg(to_jsonb(q) order by q.sort_key),'[]'::jsonb) from (${query}) q`], { cwd: repoRoot, env });
...
export const normalizeProgressSequences = (progress: readonly Json[]) => {
  ...
  if (!phases.has("derive")) phases.set("derive", new Map([["find_related", "Connecting related topics"]]));
  ...
  return [...phases]
    ...
    .map(([phase, steps]) => ({ phase, steps: [...steps].map(([key, label]) => ({ key, label })) }));
};
...
const snapshot = async (dataDir: string, env: NodeJS.ProcessEnv, progress: Json[]): Promise<Row> => {
  ...
  return {
    ...
    envelope: { registrySize: topicRows.length, membershipsPerTopic: perTopic, articlesProduced: (articles as Json[]).length },
  } satisfies Row;
};

const waitForJob = async (baseUrl: string, bearer: string, runId: string, terminal: readonly string[]) => {
  const deadline = Date.now() + 60_000;
  while (Date.now() < deadline) {
    const job = await request(baseUrl, `/v1/vaults/${fixtureIds.vault}/jobs/${runId}`, bearer) as Row;
...
const stagedWorkerScenario = async (baseUrl: string, bearer: string) => {
  await request(baseUrl, `/v1/vaults/${fixtureIds.vault}/ingest/staged-files/process`, bearer, {
    method: "POST",
    body: JSON.stringify({ job_id: fixtureIds.stagedRun, files: [{ name: "fixture.md", size: 12, hash: "golden-staged-hash", mimetype: "text/markdown" }] }),
  });
  const terminal = await waitForJob(baseUrl, bearer, fixtureIds.stagedRun, ["failed"]);
  return { trigger: terminal.trigger, status: terminal.status, phase: terminal.current_phase, phaseStatus: terminal.phase_status, steps: terminal.progress_steps, error: terminal.error } satisfies Row;
};
...
const cancelScenario = async (
  ...
  await output("psql", [databaseUrl, "-v", "ON_ERROR_STOP=1", "-c", `insert into pipeline_runs(id,vault_id,trigger,status,current_phase,phase_status) values (${sqlString(fixtureIds.cancelRun)},${sqlString(fixtureIds.vault)},'manual','pending','','')`], { cwd: repoRoot, env });
  const sse = normalizedSse(baseUrl, bearer, fixtureIds.cancelRun);
  await sleep(100);
  proxy.pauseNextResponse();
  await request(baseUrl, `/v1/vaults/${fixtureIds.vault}/compile`, bearer, { method: "POST", body: JSON.stringify({ job_id: fixtureIds.cancelRun }) });
  await Promise.race([
    proxy.waitForPaused(),
...
const lintAndCostScenarios = async (baseUrl: string, bearer: string, env: NodeJS.ProcessEnv) => {
  ...
  return {
    ...
    vaultCosts: await request(baseUrl, `/v1/vaults/${fixtureIds.vault}/costs?${window}`, bearer) as Json,
  } satisfies Row;
};
...
export const executeHarness = async (options: { readonly mode: ExecutionMode; readonly replayCassettePath: string; readonly identityFixture?: Row; readonly renderFixtures?: readonly Row[] }) => {
  ...
  try {
    await run("docker", [...docker, "down", "-v", "--remove-orphans"], { cwd: repoRoot, env, allowFailure: true });
    await run("docker", [...docker, "up", "-d", "--wait", "db"], { cwd: repoRoot, env });
    await run("uv", ["run", "alembic", "upgrade", "head"], { cwd: repoRoot, env });
    ...
    proxy = await startCassetteProxy({
      ...
      preferredTopicSlugs: options.identityFixture === undefined
        ? undefined
        : (options.identityFixture.topics as Row[]).filter((row) => row.article_status !== "archived").map((row) => String(row.slug)),
      preferredTopicTitles: options.identityFixture === undefined
        ? undefined
        : (options.identityFixture.topics as Row[]).filter((row) => row.article_status !== "archived").map((row) => String(row.title)),
      preferredRenderContents: options.renderFixtures?.flatMap((fixture) => (fixture.renderedFiles as Row[]).map((row) => String(row.content))),
    ...
    if (typeof repairPath === "string") await unlink(join(dataDir, "vaults", fixtureIds.vault, repairPath));
    ...
    const archivedTopics = (second.topics as Row[]).filter((row) => row.article_status === "archived");
    const successorFixture = archivedTopics.find((row) => row.topic_id === fixtureIds.archiveTopic);
    const nullFixture = archivedTopics.find((row) => row.topic_id === fixtureIds.archiveNoFileTopic);
    if (typeof successorFixture?.superseded_by !== "string" || nullFixture?.superseded_by !== null) {
      throw new Error("archive fixture must produce one fixed successor and one fixed null successor");
    }
    bearer = await token(baseUrl);
    const scenarios = {
      ...
      archiveSupersede: {
        ...
        archivedArticlePaths: (second.articles as Row[]).filter((row) => row.archived === true).map((row) => row.file_path),
    ...
    await run("docker", [...docker, "down", "-v", "--remove-orphans"], { cwd: repoRoot, env, allowFailure: true });
    if (process.env.GOLDENS_KEEP_RUN_DIR !== "1") await rm(runDir, { recursive: true, force: true });
  }
};

export const installPair = async (candidateCassette: string, candidateGolden: string, recordingId: string) => {
...
export const runGoldens = async (mode: Mode) => {
  ...
  const candidateCassettePath = join(stageDir, "python-compile.json");
  try {
    const recording = await executeHarness({ mode: "record", replayCassettePath: candidateCassettePath });
    ...
    try {
      ...
      await writeFile(join(reportDir, "coherence.recording.json"), `${JSON.stringify(recording.artifacts, null, 2)}\n`, "utf8");
      await writeFile(join(reportDir, "coherence.replay.json"), `${JSON.stringify(replay.artifacts, null, 2)}\n`, "utf8");
      throw error;
    }
